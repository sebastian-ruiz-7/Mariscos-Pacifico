...
module.exports = (store) => {
  ...
  const pendingsHandler = async (deliveredPendings) => {
    ...
    let tablesWithDelivered = deliveredPendings.map(
    ...
    deliveredPendings.forEach((tableInfo) => {
      const index = tablesWithDelivered.findIndex(
        (table) => table.tableNumber === tableInfo.tableNumber
      );
        ...
        if (category === "cocteles") {
          for (const size in tableInfo.order[category]) {
            for (const indexCoctel of tableInfo.order[category][size]) {
              tablesWithDelivered[index].order[category][size].coctelesSequence[
                indexCoctel
              ].delivered = true;
            }
          }
          ...
          category === "camarones" &&
          (tableInfo.order.camarones["diabla"] ||
            tableInfo.order.camarones["mojoDeAjo"] ||
            tableInfo.order.camarones["ajillo"] ||
            tableInfo.order.camarones["natural"] ||
            tableInfo.order.camarones["mantequilla"])
        ) {
          for (const flavor in tableInfo.order[category]) {
            for (const typeOfShrimp in tableInfo.order[category][flavor]) {
              tablesWithDelivered[index].order[category][flavor][
                typeOfShrimp
              ].delivered = true;
              tablesWithDelivered[index].order[category][flavor][
                typeOfShrimp
          ...
          for (const item in tableInfo.order[category]) {
            ...
            tablesWithDelivered[index].order[category][
              item
            ].notYetDelivered = 0;
          }
        }
    ...
    tablesWithDelivered.forEach((tableInfo) => {
      ...
      updateTablePromises.push(
        store.updateTable("openTables", {
  ...
  const payTable = async (id) => {
    ...
    parallelPromises.push(
      store.removeFromLastSaleInserted(
        lastSaleInserted.tableNumber,
        lastSaleInserted.fecha
      )
    );
    parallelPromises.push(
      store.add("lastSaleInserted", {
        tableNumber: bodySale.tableNumber,
        fecha: bodySale.fecha,
      })
    );
  ...
  const getPrices = async (order) => {
    ...
    for (const itemsList of categoryName) {
      ...
      for (const j in itemsList[1]) {
        ...
        if (itemIsCoctel(category)) {
          const numberOfCoctelsWithOnlyOctopus = coctelHasOnlyOctopus(
            itemsList[1][j].coctelesSequence
          );
          if (numberOfCoctelsWithOnlyOctopus > 0) {
            if (numberOfCoctelsWithOnlyOctopus == itemsList[1][j]["total"]) {
              item = {
                category: category,
                item: j,
                cantidad: numberOfCoctelsWithOnlyOctopus,
                price: index,
                onlyOctopus: true,
              }; //This line make the data structure that I want
              ...
              item = {
                ...
                item: j,
                cantidad:
                  itemsList[1][j]["total"] - numberOfCoctelsWithOnlyOctopus,
                price: index,
              }; //This line make the data structure that I want
              let price = store.getPrice(category, j); //This line makes the query to the db to get the price [This is a promise]
              pricesPromises.push(price); //I'll separate all the promises in this array in order to make this endpoint parallel rather than sequential
          ...
          if (itemsList[1][j]["cabeza"]) {
            item = {
              category: category,
              item: j,
              cantidad: itemsList[1][j]["cabeza"]["total"],
              price: index,
            };
          ...
          if (itemsList[1][j]["pelados"]) {
            item = {
              category: category,
              item: j,
              cantidad: itemsList[1][j]["pelados"]["total"],
              price: index,
            };
    ...
    products.map((changePriceAndName) => {
      ...
      if (changePriceAndName.onlyOctopus) {
        ...
        changePriceAndName["price"] = (
          parseInt(changePriceAndName["price"]) +
          parseInt(prices[prices.length - 1][0].price)
        ).toFixed(2);
        changePriceAndName["name"] = `${changePriceAndName["name"]} ${
          prices[prices.length - 1][0].name
        }`;
      } else if (
        itemIsShrimp(changePriceAndName.category, changePriceAndName.item)
      ) {
      ...
      changePriceAndName["total"] =
        changePriceAndName.cantidad * changePriceAndName.price;
    });
  ...
  const addTable = async (req) => {
    ...
    const abirMesa = await store.add("openTables", {
      tableNumber: mesa,
      fecha: dateTime,
      mesero: mesero,
      order: order,
  ...
  const updateTable = async (reqBody) => {
    const newOrder = JSON.stringify(reqBody.order);
    const changeOrder = await store.updateTable("openTables", {
  ...
  const updateTableNumber = async (reqBody) => {
    ...
    const changeOrder = await store.updateTableNumber(
      "openTables",
      { tableNumber: newTableNumber },
      reqBody.oldTableNumber
    );
  ...
  const deleteTable = async (deleteTable) => {
    const changeOrder = await store.remove("openTables", {
      tableNumber: deleteTable,
    });
    return "Procedimiento correcto";
  };
